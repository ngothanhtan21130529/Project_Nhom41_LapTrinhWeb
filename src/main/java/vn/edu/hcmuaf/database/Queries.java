...
public class Queries {
    ...
    public static final String GET_LIST_PRODUCTS_WITH_IMAGE = "SELECT p.id, p.category_id, p.product_name, p.price, p.status, p.thumbnail_id, p.sale, p.hot, p.description, p.created_at, p.updated_at, p.deleted_at, p.color, p.weight, p.opacity, p.size, p.cutting_grinding_type, p.material, i.img_url as thumbnail_url FROM products p JOIN images i ON p.thumbnail_id = i.id";
